...
class IndexSelectModel(nn.Module):
    ...
    def forward(self, x):
        ...
        output = torch.index_select(x, 1, self.indices)
        return output
...
def test_index_select():
    ...
    model = IndexSelectModel(indices=[0, 2])
    ...
    assert hasattr(quantized_model, 'index_select')
    assert not hasattr(quantized_model, 'index_select_activation_holder_quantizer')